...
> 모든 백서는 영어로만 작성한다 표이든 그림이든
> 파편화 금지, 작업후에는 처음 받은 자료(작업을 진행하는 백서/참고자료 말고)에 자신이 추가한 내용을 추가하여 사용자에게 반환한다.
   물론 틀린 내용을 수정하거나 잘못된것을 삭제하는 것은 무방하다.

**C1 — 재현성 최대 보장 (Maximum reproducibility).**
재현을 최대한 하는게 목표이다. 재현이나 수학으로 증명하지 않으면 기본적으로 왜 주장하는가? 
이런 일은 이례적인 일일것이다. 이 백서에서 거의 없어야 한다.
외부 이론은 참고하는것이지 재현하지 않으면 증명된것은 아니다. 단, 관찰값은 존중한다
...
인용한다. 따라서 강제 요건은 ① **answer-first**(페이지·섹션이 자체완결 직답으로 시작)/압축파일을 나누라는 이야기 아님
 ② **자체완결 명제**
