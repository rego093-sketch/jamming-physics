#!/usr/bin/env python3
# =============================================================================
# R11 fetch -- pin & verify the PMC open-access full text cited by the round-3
# severity/progression curation (methodology/severity_litcurate3_join.csv and
# severity_litcurate3_excluded.csv). This is the FETCH half of the R11 pass;
# it reuses the EXACT efetch / strip / open-access / id helpers of the R9 fetch
# stage (imported through R10, not re-implemented) so the pinned-snapshot format
# and the OA determination cannot drift. Writes to the SAME pinned cache
...
# only the set of cited PMCIDs (taken from the R11 CSVs) differs from R9/R10.
# =============================================================================
...
# reuse the R9 fetch helpers verbatim (via R10's fetch module): efetch_pmc,
# strip_xml, is_open_access, article_title, id_of, CACHE, RETRIEVED
_s = importlib.util.spec_from_file_location("r10_fetch", os.path.join(HERE, "r10_litcurate_fetch.py"))
F10 = importlib.util.module_from_spec(_s); _s.loader.exec_module(F10)
F = F10.F                       # the underlying R9 fetch module

CACHE = F.CACHE
RETRIEVED = F.RETRIEVED
LOG = os.path.join(CACHE, "_fetch_log_r11.json")


def cited_pmcids():
    """Union of PMCIDs referenced by the R11 round-3 join and excluded CSVs."""
    ids = collections.OrderedDict()
    for fn, col in (("severity_litcurate3_join.csv", "pmcid"),
                    ("severity_litcurate3_excluded.csv", "considered_pmcid")):
...
def main():
    ...
    ids = cited_pmcids()
    if not ids:
        print("  (no R11 curation CSVs yet; nothing to fetch)")
    ...
    log = {"source": "PubMed Central open-access (eutils.ncbi.nlm.nih.gov), full-text XML",
           "retrieved": RETRIEVED, "round": "R11", "articles": {}}
    found = 0
    for pmcid in ids:
    ...
    print(f"\n  OA articles cached for {found}/{len(ids)} R11-cited PMCIDs -> data/raw/litcurate/")
