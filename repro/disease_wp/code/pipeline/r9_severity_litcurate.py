#!/usr/bin/env python3
# =============================================================================
# R9 - curated SEVERITY / PROGRESSION literature pass  (cumulative over R5..R8)
# -----------------------------------------------------------------------------
# R8 lifted SEVERITY for the one cohort disease whose obligate dominant sequela
# is annotated in the open HPO Severity subtree, and recorded that the REMAINING
# severity/progression lift -- where it exists -- is via deliberate, human-in-
# the-loop curation from citeable published functional & survival literature.
# R9 is that curation pass, built as a gated stage IDENTICAL IN KIND to the R7
# disability join and the R8 severity join.
#
# THE A-PRIORI INCLUSION CRITERION (declared, not fitted):
#   A disease's SEVERITY (S) or PROGRESSION (P) lifts to registry-grade [L] IFF a
#   cited, peer-reviewed PMC OPEN-ACCESS source states a DISEASE-LEVEL magnitude
#   for its DOMINANT untreated sequela -- not a sub-phenotype, not a treated-cohort
#   figure, not "ranges from mild to severe" spectrum/continuum language, not a
#   comparative reference to other forms -- and that magnitude maps to the FIXED
#   BURDEN_INDEX tier by the FROZEN R3 tier function applied to the cited sentence.
#   The tier is therefore NOT hand-asserted: it is DERIVED by importing the frozen
#   R3 first_match over PATTERNS["P_progression"] / PATTERNS["S_severity"] (the
#   same a-priori cut-points R3 itself uses), and the build fails if the curated
#   sentence (i) is not re-found verbatim in the pinned OA cache, (ii) does not map
#   to the row's declared tier, or (iii) trips the R3 spectrum override.
#   (ii)-(iii) are machine-checked; "dominant untreated sequela" + "non-comparative"
#   are cited curatorial inputs recorded per row (dominant_sequela_basis), exactly
#   as R8's curation inputs were. NOT fitted to any target order.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated (max).
#   A missing/declined source NEVER downgrades a value. R9 touches ONLY S and P;
#   O/M/D stay byte-identical to R8.
#
# Self-healing / idempotent: if its own pass-tag is already present, R9 re-runs
# r5->r6->r7->r8 to rebuild a clean R8 base, then re-applies. Writes the payload
# back to burden_scores_registry.json (+ burden_residual_registry.{json,csv}) and
# prints the registry-set sha256[:12] over the four emitted artifacts.
# The R9 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# frozen R3 import (same helpers R7/R8 use) -- tier maps, composite, sensitivity
...
# declared a-priori cut-points (the EXACT BURDEN_INDEX tier table; recorded for audit)
P_CUTPOINTS = {"static/non-progressive": 0.2, "slowly progressive / progressive": 0.5,
               "rapidly progressive": 0.8, "progressive course with fatal outcome": 1.0}
S_CUTPOINTS = {"mild / asymptomatic": 0.25, "moderate": 0.5,
               "severe / debilitating": 0.75, "profound / devastating": 1.0}
DECLINE_VOCAB = {"spectrum", "comparative", "umbrella_spectrum", "historical",
                 "form_specific", "treated_cohort", "non_dominant", "sub_phenotype"}

LIT_SRC_TMPL = ("PMC open-access article {pmcid} (PMID:{pmid}), pinned text sha256[:12]={sha}, "
                "retrieved {ret}; curated dominant-sequela magnitude sentence re-found verbatim; "
                "tier derived by the frozen R3 {axis_fn} cut-points")


# ----- R3-identical spectrum overrides (so a curated sentence cannot sneak a
#       within-entity range past the tier function) -----
def spectrum_P(low):
...
def is_spectrum(axis, sentence):
    return spectrum_P(sentence.lower()) if axis == "P" else spectrum_S(sentence.lower())
...
def load_join():
    p = os.path.join(METH, "severity_litcurate_join.csv")
...
def load_excluded():
    p = os.path.join(METH, "severity_litcurate_excluded.csv")
    if not os.path.exists(p):
...
def load_pinned(pmcid):
    """Load a pinned OA snapshot and re-verify its text sha (no drift)."""
    p = os.path.join(LITC, f"{pmcid}.json")
    if not os.path.exists(p):
        print(f"  ERROR: pinned OA cache missing for {pmcid} (run r9_litcurate_fetch.py)", file=sys.stderr)
        sys.exit(3)
    art = json.load(open(p))
    if not art.get("found") or hashlib.sha256(art["text"].encode("utf-8")).hexdigest() != art.get("text_sha256"):
        print(f"  ERROR: pinned OA cache for {pmcid} fails its own sha256 (corrupt/incomplete)", file=sys.stderr)
        sys.exit(3)
    return art


def derive_tier(axis, sentence, entity):
    """Tier = frozen R3 first_match over the cited sentence (a-priori cut-points)."""
    hit = first_match(sentence, PATTERNS[AXIS_PATTERN[axis]])
    if not hit:
        print(f"  ERROR: curated sentence for {entity} ({axis}) matches no R3 tier pattern", file=sys.stderr)
        sys.exit(4)
    if is_spectrum(axis, sentence):
        print(f"  ERROR: curated sentence for {entity} ({axis}) trips the R3 spectrum override "
              f"(within-entity range); cannot lift", file=sys.stderr)
        sys.exit(4)
    return hit  # (value, label, phrase)
...
def main():
    ...
    # self-healing: if R9 already applied, rebuild a clean R8 base from r5..r8
    if "R9_litcurate_severity_progression" in base.get("registry_passes", []):
        ...
        base = json.load(open(OUT_SCORES_J))
    if "R8_open_severity" not in base.get("registry_passes", []):
        print("  ERROR: R9 expects an R8 base (registry_passes missing R8_open_severity)", file=sys.stderr)
        sys.exit(3)
    ...
    for jr in join:
        ...
        join_audit.append({"entity": entity, "cui": cui, "axis": axis, "pmcid": jr["pmcid"],
                           ...
                           "open_access": True})
        join_by_cui_axis[(cui, axis)] = tier_val
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement considered and declined "
    ...
    # ---------------- recompute composite / coverage / rankability (identical to R8) ----------------
    for r in recs:
        comp = r["components"]
    ...
    payload = {
        ...
        "phase": "R9 (curated severity/progression literature; cumulative over R5+R6+R7+R8)",
        ...
        "method": ("methodology/BURDEN_INDEX.md + GBD2013 disability weights (R7) + PMC mortality "
                   "corroboration (R7) + HPO Severity-modifier curation join (R8) + curated PMC-OA "
                   "severity/progression literature join (R9)"),
        "derived_from": ("burden_scores_registry.json (R5+R6+R7+R8) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate_join.csv + methodology/severity_litcurate_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R9_litcurate_severity_progression"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R9: S/P <- curated PMC OPEN-ACCESS literature, applied ONLY "
            "through a cited DOMINANT-sequela curation join (methodology/severity_litcurate_join.csv); grade [L] "
            "(curation join, like R7/R8). A disease's S or P lifts iff a cited OA source states a DISEASE-LEVEL "
            "magnitude for its dominant untreated sequela (non-spectrum, non-comparative, non-treated-cohort), and "
            "the FROZEN R3 tier function applied to the verbatim cited sentence yields the declared tier. The tier "
            "is DERIVED (not hand-asserted); each cited sentence is re-found verbatim in the pinned OA cache at build "
            "time or the build fails; the R3 spectrum override is enforced. Every declined statement is recorded with "
            "a decline_class + per-disease reason (methodology/severity_litcurate_excluded.csv). Registry [L] "
            "supersedes [H]/[O], existing [L] corroborated (max, add-only). No inference; no new cut-points."),
        ...
        "litcurate_inclusion_criterion": (
            "S or P -> [L] for a cohort disease IFF a cited PMC OPEN-ACCESS source states a DISEASE-LEVEL magnitude "
            "for its DOMINANT untreated sequela -- (i) NOT a sub-phenotype / non-dominant feature, (ii) NOT a "
            "treated-cohort figure, (iii) NOT spectrum/continuum/'ranges from mild to severe' language, (iv) NOT a "
            "comparative reference to other forms -- and the FROZEN R3 tier function (first_match over "
            "PATTERNS['P_progression'] / ['S_severity']; same a-priori cut-points R3 uses) applied to the verbatim "
            "cited sentence yields the declared tier. (iii) is machine-enforced by the R3 spectrum override; the "
            "sentence is re-found verbatim in the pinned OA cache; (i),(ii),(iv) are cited curatorial inputs recorded "
            "in dominant_sequela_basis / exclusion_reason. A-priori, not fitted to any target order."),
        ...
        "litcurate_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                             "retrieved": RETRIEVED, "cited_articles": sorted({a["pmcid"] for a in join_audit}
        ...
        "litcurate_join": join_audit,
        "litcurate_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R9 (curated severity/progression literature; cumulative over R5+R6+R7+R8)",
        "derived_from": "burden_scores_registry.json (R5..R9) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R9 curated severity/progression literature pass (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R9 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
