#!/usr/bin/env python3
# =============================================================================
# R10 fetch -- pin & verify the PMC open-access full text cited by the round-2
# severity/progression curation (methodology/severity_litcurate2_join.csv and
# severity_litcurate2_excluded.csv). This is the FETCH half of the R10 pass;
# it reuses the EXACT efetch / strip / open-access / id helpers of the R9 fetch
# stage (imported, not re-implemented) so the pinned-snapshot format and the OA
# determination cannot drift. Writes to the SAME pinned cache (data/raw/litcurate/)
# and is idempotent: a snapshot whose stored text still matches its own sha256 is
# left untouched. No tuning, no parsing changes -- only the set of cited PMCIDs
# (taken from the R10 CSVs) differs from R9.
# =============================================================================
...
# reuse R9 fetch helpers verbatim (efetch_pmc, strip_xml, is_open_access, ...)
_s = importlib.util.spec_from_file_location("r9_fetch", os.path.join(HERE, "r9_litcurate_fetch.py"))
F = importlib.util.module_from_spec(_s); _s.loader.exec_module(F)

CACHE = F.CACHE
RETRIEVED = F.RETRIEVED
LOG = os.path.join(CACHE, "_fetch_log_r10.json")


def cited_pmcids():
    """Union of PMCIDs referenced by the R10 round-2 join and excluded CSVs."""
    ids = collections.OrderedDict()
    for fn, col in (("severity_litcurate2_join.csv", "pmcid"),
                    ("severity_litcurate2_excluded.csv", "considered_pmcid")):
...
def main():
    ...
    ids = cited_pmcids()
    if not ids:
        print("  (no R10 curation CSVs yet; nothing to fetch)")
    ...
    log = {"source": "PubMed Central open-access (eutils.ncbi.nlm.nih.gov), full-text XML",
           "retrieved": RETRIEVED, "round": "R10", "articles": {}}
    found = 0
    for pmcid in ids:
    ...
    print(f"\n  OA articles cached for {found}/{len(ids)} R10-cited PMCIDs -> data/raw/litcurate/")
