#!/usr/bin/env python3
# =============================================================================
# R15 fetch -- pin & verify the PMC open-access full text cited by the round-7
# severity/progression curation (methodology/severity_litcurate7_join.csv and
# severity_litcurate7_excluded.csv). This is the FETCH half of the R15 pass;
# it reuses the EXACT efetch / strip / open-access / id helpers of the R9 fetch
# stage (imported through R14 -> R13 -> R12 -> R11 -> R10, not re-implemented) so the
# pinned-snapshot format and the OA determination cannot drift. Writes to the
# SAME pinned cache (data/raw/litcurate/) and is idempotent: a snapshot whose
# stored text still matches its own sha256 is left untouched. No tuning, no
# parsing changes -- only the set of cited PMCIDs (taken from the R15 CSVs)
# differs from R9/R10/R11/R12/R13/R14.
# =============================================================================
...
# reuse the R9 fetch helpers verbatim (via R14 -> R13 -> R12 -> R11 -> R10's fetch module):
# efetch_pmc, strip_xml, is_open_access, article_title, id_of, CACHE, RETRIEVED
_s = importlib.util.spec_from_file_location("r14_fetch", os.path.join(HERE, "r14_litcurate_fetch.py"))
F14 = importlib.util.module_from_spec(_s); _s.loader.exec_module(F14)
F = F14.F                       # the underlying R9 fetch module (through R10/R11/R12/R13/R14)

CACHE = F.CACHE
RETRIEVED = F.RETRIEVED
LOG = os.path.join(CACHE, "_fetch_log_r15.json")


def cited_pmcids():
    """Union of PMCIDs referenced by the R15 round-7 join and excluded CSVs."""
    ids = collections.OrderedDict()
    for fn, col in (("severity_litcurate7_join.csv", "pmcid"),
                    ("severity_litcurate7_excluded.csv", "considered_pmcid")):
...
def main():
    ...
    ids = cited_pmcids()
    if not ids:
        print("  (no R15 curation CSVs yet; nothing to fetch)")
    ...
    log = {"source": "PubMed Central open-access (eutils.ncbi.nlm.nih.gov), full-text XML",
           "retrieved": RETRIEVED, "round": "R15", "articles": {}}
    found = 0
    for pmcid in ids:
    ...
    print(f"\n  OA articles cached for {found}/{len(ids)} R15-cited PMCIDs -> data/raw/litcurate/")
