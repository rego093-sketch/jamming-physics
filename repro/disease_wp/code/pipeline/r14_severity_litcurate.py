#!/usr/bin/env python3
# =============================================================================
# R14 - curated SEVERITY / PROGRESSION literature pass, ROUND 6
#        (cumulative over R5..R13)
# -----------------------------------------------------------------------------
# R9/R10/R11/R12/R13 lifted S/P for the cohort diseases whose dominant untreated
# sequela had a citeable DISEASE-LEVEL magnitude in a PMC open-access source,
# recorded every declined statement, and left the asymmetric discipline in
# place: lift only where defensible, decline transparently, never fish for an
# order-lock. R14 is a SIXTH curation round over the SAME a-priori inclusion
# criterion and the SAME FROZEN R3 tier function -- it does NOT touch R9's,
# R10's, R11's, R12's or R13's recorded artifacts (methodology/
# severity_litcurate{,2,3,4,5}_join.csv, *_excluded.csv) or their recorded
# registry-set shas; it adds a fresh, separately-auditable join/excluded set
# (methodology/severity_litcurate6_join.csv, *_excluded.csv) on top of the R13
# registry, exactly as R13 was a separate cumulative stage over R12 and R7 was
# over R6.
#
...
#   THROUGH R13->R12->R11->R10->R9; no new cut-points), the sentence is re-found
#   verbatim in the pinned OA cache at build time or the build fails, and the R3
#   spectrum override is enforced. All of derive_tier / is_spectrum / the
#   cut-point tables / the decline vocabulary are IMPORTED (through
#   r13_severity_litcurate, which imports r12_severity_litcurate, which imports
#   r11_severity_litcurate, which imports r10_severity_litcurate, which imports
#   r9_severity_litcurate) so they cannot drift.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated
#   (max). A missing/declined source NEVER downgrades a value. R14 touches ONLY
#   S and P; O/M/D stay byte-identical to R13.
#
# Self-healing / idempotent: if its own pass-tag is already present, R14 re-runs
# r5..r13 to rebuild a clean R13 base, then re-applies. Writes the payload back
...
# The R14 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# ---- import R13's helpers (which import R12's, R11's, R10's, R9's PURE helpers)
#      so the frozen tier / spectrum / decline logic is literally the same object
#      (cannot drift); r13.main / r12.main / r11.main / r10.main / r9.main are __main__-guarded ----
_s13 = importlib.util.spec_from_file_location("r13_litc", os.path.join(HERE, "r13_severity_litcurate.py"))
r13 = importlib.util.module_from_spec(_s13); _s13.loader.exec_module(r13)
W = r13.W
GRADE_RANK = r13.GRADE_RANK
floor_grade = r13.floor_grade
weighted_burden = r13.weighted_burden
spearman = r13.spearman
RANK_MIN = r13.RANK_MIN
AXIS_PATTERN = r13.AXIS_PATTERN
P_CUTPOINTS = r13.P_CUTPOINTS
S_CUTPOINTS = r13.S_CUTPOINTS
DECLINE_VOCAB = r13.DECLINE_VOCAB
LIT_SRC_TMPL = r13.LIT_SRC_TMPL
derive_tier = r13.derive_tier          # frozen R3 first_match + spectrum guard
load_pinned = r13.load_pinned          # verifies pinned OA cache + its own sha
_append_once = r13._append_once


def load_join6():
    p = os.path.join(METH, "severity_litcurate6_join.csv")
...
def load_excluded6():
    p = os.path.join(METH, "severity_litcurate6_excluded.csv")
    if not os.path.exists(p):
...
def main():
    join = load_join6()
    excluded = load_excluded6()

    base = json.load(open(OUT_SCORES_J))
    # self-healing: if R14 already applied, rebuild a clean R13 base from r5..r13
    if "R14_litcurate6" in base.get("registry_passes", []):
        for stage in ("r5_accession_apply.py", "r6_naturalhistory_registry.py",
                      ...
                      "r13_severity_litcurate.py"):
            subprocess.run([sys.executable, os.path.join(HERE, stage)],
                           check=True, stdout=subprocess.DEVNULL)
        base = json.load(open(OUT_SCORES_J))
    if "R13_litcurate5" not in base.get("registry_passes", []):
        print("  ERROR: R14 expects an R13 base (registry_passes missing R13_litcurate5)", file=sys.stderr)
        sys.exit(3)
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement (round 6) considered and declined "
    ...
    # ---------------- recompute composite / coverage / rankability (identical to R8..R12) ----------------
    for r in recs:
        comp = r["components"]
    ...
    payload = {
        ...
        "phase": "R14 (curated severity/progression literature, round 6; cumulative over R5+R6+R7+R8+R9+R10+R11+R12+R13)",
        "investigation_only": base.get("investigation_only", True),
        "generated": RETRIEVED,
        "method": (base.get("method", "")
                   + " + curated PMC-OA severity/progression literature join, round 6 (R14)"),
        "derived_from": ("burden_scores_registry.json (R5..R13) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate6_join.csv + methodology/severity_litcurate6_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R14_litcurate6"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R14: SIXTH curated PMC OPEN-ACCESS S/P round over the SAME "
            "a-priori inclusion criterion and the SAME frozen R3 tier function as R9/R10/R11/R12/R13, applied through a "
            "fresh, separately-auditable curation join (methodology/severity_litcurate6_join.csv) plus a recorded "
            "decline set (methodology/severity_litcurate6_excluded.csv). R14 does NOT modify R9's, R10's, R11's, "
            "R12's or R13's recorded join/excluded CSVs or their recorded registry-set shas; it is a separate cumulative stage "
            "over the R13 registry, exactly as R13 was a separate cumulative stage over R12 and R7 over R6. "
            "derive_tier / is_spectrum / the cut-point tables / the decline vocabulary are IMPORTED (through "
            "r13_severity_litcurate, which imports r12_severity_litcurate, which imports r11_severity_litcurate, which "
            "imports r10_severity_litcurate, which imports r9_severity_litcurate) so they cannot drift. Registry [L] supersedes [H]/[O], existing [L] "
            "corroborated (max, add-only). No inference; no new cut-points."),
        ...
        "litcurate6_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                              "retrieved": RETRIEVED,
        ...
        "litcurate6_join": join_audit,
        "litcurate6_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R14 (curated severity/progression literature, round 6; cumulative over R5..R13)",
        "derived_from": "burden_scores_registry.json (R5..R14) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R14 curated severity/progression literature pass, ROUND 6 (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R14 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
