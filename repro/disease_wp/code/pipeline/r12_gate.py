...
# -*- coding: utf-8 -*-
"""
R12 gate -- verifies the curated SEVERITY / PROGRESSION literature pass, ROUND 4,
independently of any recorded digest. It:
  * proves the r5->r6->r7->r8->r9->r10->r11->r12 chain is deterministic (2x
    identical registry-set sha);
  * re-anchors every round-4 curation-join row in the PINNED PMC-OA cache (cited
  ...
  * confirms every round-4 declined statement is recorded with a decline_class
    (declared vocabulary) + a per-disease reason, is itself re-found in the pinned
    cache, and did NOT lift that disease's S/P to [L];
  * confirms add-only (no grade weakened / no scored value removed vs the R11 base)
    and that the ONLY axes R12 touches are S and P, changed ONLY for the round-4
    joined diseases (O/M/D byte-identical to R11);
  * proves R12's severity lifts PRODUCE EXACTLY ONE new order-lock: the lock set
    grows 4 -> 5, the added disease is EXACTLY Duchenne muscular dystrophy
    (C0013264), and Duchenne's severity grade is what changes ([H] -> [L]) to
    complete the lock (its onset/progression/mortality/disability were already
    registry-[L]); the second lift (Tyrosinemia type I, grade-only) does NOT lock;
  * re-derives composites / rankability / residual / order-lock-rule / sensitivity;
  * confirms every round-4 cited source is open-access and each lift is fully
    source-bound (no inference);
  * confirms the R3/R4 BANKED files stay byte-identical and the engine pin is drift-0;
  * confirms R9's, R10's AND R11's OWN recorded artifacts are untouched (their
    join/excluded CSV digests are preserved -- R12 is additive, not a mutation of
    any prior round).

Living code, NOT in the frozen engine pin.  Out: reports/r12.gate.json
"""
...
JOIN4 = os.path.join(METH, "severity_litcurate4_join.csv")
EXCL4 = os.path.join(METH, "severity_litcurate4_excluded.csv")
...
# R9's, R10's AND R11's own recorded artifacts -- R12 must NOT modify these (additive, not a mutation)
PRIOR_RECORDED = {
...
DMD = "C0013264"   # Duchenne muscular dystrophy -- the disease R12's severity lift is expected to order-lock
...
def main():
    ...
    # ---- 1) determinism: r5..r12 chain twice -> identical registry-set sha ----
    run(R5); run(R6); run(R7); run(R8); run(R9); run(R10); run(R11); run(R12); s1 = registry_set_sha()
    run(R5); run(R6); run(R7); run(R8); run(R9); run(R10); run(R11); run(R12); s2 = registry_set_sha()
    checks.append(("determinism_chain_2x", s1 == s2,
                   f"r5->...->r11->r12 chain registry-set sha {s1} == {s2}"))

    # capture R11 base (pre round-4), then R12
    run(R5); run(R6); run(R7); run(R8); run(R9); run(R10); run(R11); P11 = json.load(open(SCORES_J))
    r11_state = axis_state(P11); r11_lock = lock_state(P11)
    run(R12); P = json.load(open(SCORES_J)); r12_state = axis_state(P); r12_lock = lock_state(P)
    recs = P["records"]
    ...
    # ---- 3) R12 builds on a real R11 base (cumulative) ----
    passes = P.get("registry_passes", [])
    base_ok = (passes[-1] == "R12_litcurate4"
    ...
    checks.append(("cumulative_over_r11", base_ok, f"registry_passes = {passes}"))

    # ---- 4) round-4 curation join re-anchored in pinned OA cache + tier from frozen R3 + non-spectrum ----
    join_rows = list(csv.DictReader(open(JOIN4, newline="")))
    excl_csv = collections.defaultdict(list)
    for r in csv.DictReader(open(EXCL4, newline="")):
        excl_csv[r["cui"]].append(r)
    audit = {(a["cui"], a["axis"]): a for a in P.get("litcurate4_join", [])}
    jbad = []
    for jr in join_rows:
        ...
        if not a:
            jbad.append(f"{entity}/{axis} missing from litcurate4_join audit")
        elif abs(a["tier"] - hit[0]) > 1e-9:
            jbad.append(f"{entity}/{axis} audit tier mismatch")
    checks.append(("litcurate4_join_anchored_cited", not jbad,
                   f"{len(join_rows)} round-4 join(s): cited sentence re-found verbatim in pinned OA cache, "
                   f"source open-access, basis present, tier == frozen-R3-derived, non-spectrum ({len(jbad)} bad)"))

    # ---- 5) tier cut-points are the BURDEN_INDEX a-priori map (recorded) + every round-4 lifted axis == derived ----
    ...
    for (cui, axis) in join_keys:
        ...
        prior = (r11_state[cui][axis][0] or 0.0)
        if ax["grade"] != "[L]":
            scut_bad.append(f"{jr['entity']} {axis} not [L] after round-4 lift")
    ...
    checks.append(("litcurate4_cutpoints_a_priori", cut_ok and not scut_bad,
                   f"recorded P/S cut-points equal the BURDEN_INDEX map; every round-4 curated S/P == frozen-R3 "
                   f"derived tier ({len(scut_bad)} bad)"))

    # ---- 6) round-4 declines recorded + not lifted to [L] via curation ----
    ebad = []
    rec_excl = {(e["cui"], e["axis"], e["considered_pmcid"]) for e in P.get("litcurate4_excluded", [])}
    for cui, rows in excl_csv.items():
        ...
        for x in rows:
            ...
            # the declined disease must NOT have had THIS axis lifted via round-4 curation
            if (cui, x["axis"]) in join_keys:
                ebad.append(f"{x['entity']} declined but {x['axis']} lifted via round-4 curation")
    checks.append(("litcurate4_excluded_recorded", not ebad,
                   f"{sum(len(v) for v in excl_csv.values())} round-4 decline(s): in cohort, decline_class in "
                   f"vocab, reason present, statement re-found in pinned cache, axis not curation-lifted ({len(ebad)} bad)"))

    # ---- 7) add-only: no grade weakened, no scored value removed, vs R11 ----
    downgrade = []
    for cui in r12_state:
        for ax in W:
            v11, g11 = r11_state[cui][ax]; v12, g12 = r12_state[cui][ax]
            if GR[g12] < GR[g11]:
                downgrade.append(f"{cui}/{ax} {g11}->{g12}")
            if v11 is not None and v12 is None:
                downgrade.append(f"{cui}/{ax} value dropped")
    checks.append(("addonly_no_downgrade", not downgrade,
                   f"no axis grade weakened and no scored value removed vs R11 ({len(downgrade)} violations)"))

    # ---- 8) R12 touches ONLY S/P, changed ONLY for round-4 joined diseases (0 illicit) ----
    illicit = []
    for cui in r12_state:
        for ax in ("O", "M", "D"):
            if r11_state[cui][ax] != r12_state[cui][ax]:
                illicit.append(f"{cui}/{ax} changed (R12 must touch only S/P)")
        for ax in ("S", "P"):
            if r11_state[cui][ax] != r12_state[cui][ax] and (cui, ax) not in join_keys:
                illicit.append(f"{cui}/{ax} changed but not in round-4 join")
    checks.append(("sp_lift_only_where_joined", not illicit,
                   f"O/M/D identical to R11; S/P changed only for the {len(join_keys)} round-4 joined "
                   f"(disease,axis) pair(s) ({len(illicit)} illicit)"))

    # ---- 9) EXACTLY ONE new order-lock: set grows 4->5, added disease IS Duchenne MD, its S grade [H]->[L] ----
    ...
    added = locked12 - locked11
    removed = locked11 - locked12
    lock_ok = (r11_lockn == 4 and r12_lockn == 5
               and added == {DMD} and not removed
               and r11_lock.get(DMD) is False and r12_lock.get(DMD) is True
               # the lift that completes the lock is severity: Duchenne S [H] -> [L]
               and r11_state[DMD]["S"][1] == "[H]" and r12_state[DMD]["S"][1] == "[L]"
               # and Duchenne's other scored axes were ALREADY [L] in the R11 base (S is the only completer)
               and all(r11_state[DMD][ax][1] in ("[L]", "[V]")
                       for ax in W if r11_state[DMD][ax][0] is not None and ax != "S"))
    new_lock_entities = sorted(rec_by_cui[c]["entity"] for c in added)
    checks.append(("new_order_lock_is_duchenne", lock_ok,
                   f"order-lock set grows {r11_lockn} -> {r12_lockn}; the single added lock is EXACTLY Duchenne "
                   f"muscular dystrophy {new_lock_entities} via S [H]->[L] (onset/progression/mortality/disability "
                   f"already [L]); the grade-only second lift does not lock; no lock removed"))
    ...
    # ---- 14) every round-4 cited source open-access + lift fully source-bound (no inference) ----
    oa_bad = []
    cited = {a["pmcid"] for a in P.get("litcurate4_join", [])} | {e["considered_pmcid"] for e in P.get("litcurate4_excluded", [])}
    for pmcid in cited:
    ...
    # every round-4 [L] lift must be bound to a round-4 join row
    for (cui, axis) in join_keys:
        ax = rec_by_cui[cui]["components"][axis]
        if ax["grade"] != "[L]":
            oa_bad.append(f"{rec_by_cui[cui]['entity']}/{axis} round-4 join but not [L]")
    crit_ok = bool(P.get("litcurate_inclusion_criterion"))
    checks.append(("litcurate4_sources_open_access_and_bound", not oa_bad and crit_ok,
                   f"all {len(cited)} round-4 cited sources open-access + sha-pinned; every round-4 S/P [L] is bound "
                   f"to a join row (no inference); inclusion criterion declared ({len(oa_bad)} bad)"))

    # ---- 15) R9's, R10's AND R11's OWN recorded artifacts preserved (R12 additive, not a mutation) ----
    prior_preserved = []
    ...
    checks.append(("prior_round_artifacts_preserved", not prior_preserved,
                   f"R9 + R10 + R11 recorded join/excluded CSVs byte-identical to their digests "
                   f"({len(prior_preserved)} modified) -- R12 is additive"))
    ...
    report = {
        "phase": "R12", "kind": "investigation_gate",
        "title": "curated severity/progression literature pass, round 4 (PMC-OA dominant-sequela join) — severity lift that order-locks Duchenne muscular dystrophy",
        ...
        "inputs": {
            ...
            "litcurate4_join_csv": {"path": "methodology/severity_litcurate4_join.csv", "sha12": sha12(JOIN4)},
            "litcurate4_excluded_csv": {"path": "methodology/severity_litcurate4_excluded.csv", "sha12": sha12(EXCL4)},
            "litcurate_fetch_log_r12": {"path": "data/raw/litcurate/_fetch_log_r12.json", "sha12": sha12(os.path.join(LITC, "_fetch_log_r12.json"))},
        },
        "lift_counts": P.get("lift_counts"),
        "axis_grade_coverage_registry": P.get("axis_grade_coverage_registry"),
        "litcurate4_join": P.get("litcurate4_join"),
        "litcurate4_excluded": P.get("litcurate4_excluded"),
    ...
    json.dump(report, open(os.path.join(ROOT, "reports", "r12.gate.json"), "w"), indent=2, ensure_ascii=False)

    print(f"R12 GATE: {verdict}  ({passed}/{total})")
