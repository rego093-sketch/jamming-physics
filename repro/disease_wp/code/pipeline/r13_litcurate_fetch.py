#!/usr/bin/env python3
# =============================================================================
# R13 fetch -- pin & verify the PMC open-access full text cited by the round-5
# severity/progression curation (methodology/severity_litcurate5_join.csv and
# severity_litcurate5_excluded.csv). This is the FETCH half of the R13 pass;
# it reuses the EXACT efetch / strip / open-access / id helpers of the R9 fetch
# stage (imported through R12 -> R11 -> R10, not re-implemented) so the pinned-
...
# changes -- only the set of cited PMCIDs (taken from the R13 CSVs) differs from
# R9/R10/R11/R12.
# =============================================================================
...
# reuse the R9 fetch helpers verbatim (via R12 -> R11 -> R10's fetch module):
# efetch_pmc, strip_xml, is_open_access, article_title, id_of, CACHE, RETRIEVED
_s = importlib.util.spec_from_file_location("r12_fetch", os.path.join(HERE, "r12_litcurate_fetch.py"))
F12 = importlib.util.module_from_spec(_s); _s.loader.exec_module(F12)
F = F12.F                       # the underlying R9 fetch module (through R10/R11/R12)

CACHE = F.CACHE
RETRIEVED = F.RETRIEVED
LOG = os.path.join(CACHE, "_fetch_log_r13.json")


def cited_pmcids():
    """Union of PMCIDs referenced by the R13 round-5 join and excluded CSVs."""
    ids = collections.OrderedDict()
    for fn, col in (("severity_litcurate5_join.csv", "pmcid"),
                    ("severity_litcurate5_excluded.csv", "considered_pmcid")):
...
def main():
    ...
    ids = cited_pmcids()
    if not ids:
        print("  (no R13 curation CSVs yet; nothing to fetch)")
    ...
    log = {"source": "PubMed Central open-access (eutils.ncbi.nlm.nih.gov), full-text XML",
           "retrieved": RETRIEVED, "round": "R13", "articles": {}}
    found = 0
    for pmcid in ids:
    ...
    print(f"\n  OA articles cached for {found}/{len(ids)} R13-cited PMCIDs -> data/raw/litcurate/")
