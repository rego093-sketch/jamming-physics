#!/usr/bin/env python3
# =============================================================================
# R11 - curated SEVERITY / PROGRESSION literature pass, ROUND 3
#        (cumulative over R5..R10)
# -----------------------------------------------------------------------------
# R9/R10 lifted S/P for the cohort diseases whose dominant untreated sequela had
# a citeable DISEASE-LEVEL magnitude in a PMC open-access source, recorded every
...
# R11 is a THIRD curation round over the SAME a-priori inclusion criterion and
# the SAME FROZEN R3 tier function -- it does NOT touch R9's or R10's recorded
# artifacts (methodology/severity_litcurate{,2}_join.csv, *_excluded.csv) or
# their recorded registry-set shas; it adds a fresh, separately-auditable
# join/excluded set (methodology/severity_litcurate3_join.csv, *_excluded.csv)
# on top of the R10 registry, exactly as R10 was a separate cumulative stage
# over R9 and R7 was over R6.
#
...
#   THROUGH R10->R9; no new cut-points), the sentence is re-found verbatim in the
#   pinned OA cache at build time or the build fails, and the R3 spectrum override
#   is enforced. All of derive_tier / is_spectrum / the cut-point tables / the
#   decline vocabulary are IMPORTED (through r10_severity_litcurate, which imports
#   r9_severity_litcurate) so they cannot drift.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated
#   (max). A missing/declined source NEVER downgrades a value. R11 touches ONLY
#   S and P; O/M/D stay byte-identical to R10.
#
# Self-healing / idempotent: if its own pass-tag is already present, R11 re-runs
# r5..r10 to rebuild a clean R10 base, then re-applies. Writes the payload back to
...
# The R11 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# ---- import R10's helpers (which import R9's PURE helpers) so the frozen tier /
#      spectrum / decline logic is literally the same object (cannot drift);
#      r10.main / r9.main are __main__-guarded ----
_s10 = importlib.util.spec_from_file_location("r10_litc", os.path.join(HERE, "r10_severity_litcurate.py"))
r10 = importlib.util.module_from_spec(_s10); _s10.loader.exec_module(r10)
W = r10.W
GRADE_RANK = r10.GRADE_RANK
floor_grade = r10.floor_grade
weighted_burden = r10.weighted_burden
spearman = r10.spearman
RANK_MIN = r10.RANK_MIN
AXIS_PATTERN = r10.AXIS_PATTERN
P_CUTPOINTS = r10.P_CUTPOINTS
S_CUTPOINTS = r10.S_CUTPOINTS
DECLINE_VOCAB = r10.DECLINE_VOCAB
LIT_SRC_TMPL = r10.LIT_SRC_TMPL
derive_tier = r10.derive_tier          # frozen R3 first_match + spectrum guard
load_pinned = r10.load_pinned          # verifies pinned OA cache + its own sha
_append_once = r10._append_once


def load_join3():
    p = os.path.join(METH, "severity_litcurate3_join.csv")
...
def load_excluded3():
    p = os.path.join(METH, "severity_litcurate3_excluded.csv")
    if not os.path.exists(p):
...
def main():
    join = load_join3()
    excluded = load_excluded3()

    base = json.load(open(OUT_SCORES_J))
    # self-healing: if R11 already applied, rebuild a clean R10 base from r5..r10
    if "R11_litcurate3" in base.get("registry_passes", []):
        ...
        base = json.load(open(OUT_SCORES_J))
    if "R10_litcurate2" not in base.get("registry_passes", []):
        print("  ERROR: R11 expects an R10 base (registry_passes missing R10_litcurate2)", file=sys.stderr)
        sys.exit(3)
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement (round 3) considered and declined "
    ...
    # ---------------- recompute composite / coverage / rankability (identical to R8/R9/R10) ----------------
    for r in recs:
        comp = r["components"]
    ...
    payload = {
        ...
        "phase": "R11 (curated severity/progression literature, round 3; cumulative over R5+R6+R7+R8+R9+R10)",
        "investigation_only": base.get("investigation_only", True),
        "generated": RETRIEVED,
        "method": (base.get("method", "")
                   + " + curated PMC-OA severity/progression literature join, round 3 (R11)"),
        "derived_from": ("burden_scores_registry.json (R5..R10) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate3_join.csv + methodology/severity_litcurate3_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R11_litcurate3"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R11: THIRD curated PMC OPEN-ACCESS S/P round over the SAME "
            "a-priori inclusion criterion and the SAME frozen R3 tier function as R9/R10, applied through a fresh, "
            "separately-auditable curation join (methodology/severity_litcurate3_join.csv) plus a recorded decline "
            "set (methodology/severity_litcurate3_excluded.csv). R11 does NOT modify R9's or R10's recorded "
            "join/excluded CSVs or their recorded registry-set shas; it is a separate cumulative stage over the R10 "
            "registry, exactly as R10 was a separate cumulative stage over R9 and R7 over R6. derive_tier / "
            "is_spectrum / the cut-point tables / the decline vocabulary are IMPORTED (through r10_severity_litcurate, "
            "which imports r9_severity_litcurate) so they cannot drift. Registry [L] supersedes [H]/[O], existing [L] "
        ...
        "litcurate3_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                              "retrieved": RETRIEVED,
        ...
        "litcurate3_join": join_audit,
        "litcurate3_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R11 (curated severity/progression literature, round 3; cumulative over R5..R10)",
        "derived_from": "burden_scores_registry.json (R5..R11) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R11 curated severity/progression literature pass, ROUND 3 (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R11 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
