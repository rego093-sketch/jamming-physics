#!/usr/bin/env python3
# =============================================================================
# R12 fetch -- pin & verify the PMC open-access full text cited by the round-4
# severity/progression curation (methodology/severity_litcurate4_join.csv and
# severity_litcurate4_excluded.csv). This is the FETCH half of the R12 pass;
# it reuses the EXACT efetch / strip / open-access / id helpers of the R9 fetch
# stage (imported through R11 -> R10, not re-implemented) so the pinned-snapshot
# format and the OA determination cannot drift. Writes to the SAME pinned cache
# (data/raw/litcurate/) and is idempotent: a snapshot whose stored text still
# matches its own sha256 is left untouched. No tuning, no parsing changes --
# only the set of cited PMCIDs (taken from the R12 CSVs) differs from R9/R10/R11.
# =============================================================================
...
# reuse the R9 fetch helpers verbatim (via R11 -> R10's fetch module): efetch_pmc,
# strip_xml, is_open_access, article_title, id_of, CACHE, RETRIEVED
_s = importlib.util.spec_from_file_location("r11_fetch", os.path.join(HERE, "r11_litcurate_fetch.py"))
F11 = importlib.util.module_from_spec(_s); _s.loader.exec_module(F11)
F = F11.F                       # the underlying R9 fetch module (through R10/R11)

CACHE = F.CACHE
RETRIEVED = F.RETRIEVED
LOG = os.path.join(CACHE, "_fetch_log_r12.json")


def cited_pmcids():
    """Union of PMCIDs referenced by the R12 round-4 join and excluded CSVs."""
    ids = collections.OrderedDict()
    for fn, col in (("severity_litcurate4_join.csv", "pmcid"),
                    ("severity_litcurate4_excluded.csv", "considered_pmcid")):
...
def main():
    ...
    ids = cited_pmcids()
    if not ids:
        print("  (no R12 curation CSVs yet; nothing to fetch)")
    ...
    log = {"source": "PubMed Central open-access (eutils.ncbi.nlm.nih.gov), full-text XML",
           "retrieved": RETRIEVED, "round": "R12", "articles": {}}
    found = 0
    for pmcid in ids:
    ...
    print(f"\n  OA articles cached for {found}/{len(ids)} R12-cited PMCIDs -> data/raw/litcurate/")
