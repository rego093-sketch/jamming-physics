#!/usr/bin/env python3
# =============================================================================
# R15 - curated SEVERITY / PROGRESSION literature pass, ROUND 7
#        (cumulative over R5..R14)
# -----------------------------------------------------------------------------
# R9..R14 lifted S/P for the cohort diseases whose dominant untreated sequela
# had a citeable DISEASE-LEVEL magnitude in a PMC open-access source, recorded
# every declined statement, and left the asymmetric discipline in place: lift
# only where defensible, decline transparently, never fish for an order-lock.
# R15 is a SEVENTH curation round over the SAME a-priori inclusion criterion and
# the SAME FROZEN R3 tier function -- it does NOT touch R9..R14's recorded
# artifacts (methodology/severity_litcurate{,2,3,4,5,6}_join.csv, *_excluded.csv)
# or their recorded registry-set shas; it adds a fresh, separately-auditable
# join/excluded set (methodology/severity_litcurate7_join.csv, *_excluded.csv)
# on top of the R14 registry, exactly as R14 was a separate cumulative stage
# over R13 and R7 was over R6.
#
...
#   THROUGH R14->R13->R12->R11->R10->R9; no new cut-points), the sentence is
#   re-found verbatim in the pinned OA cache at build time or the build fails,
#   and the R3 spectrum override is enforced. All of derive_tier / is_spectrum /
#   the cut-point tables / the decline vocabulary are IMPORTED (through
#   r14_severity_litcurate, which imports r13_severity_litcurate, ... which
#   imports r9_severity_litcurate) so they cannot drift.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated
#   (max). A missing/declined source NEVER downgrades a value. R15 touches ONLY
#   S and P; O/M/D stay byte-identical to R14.
#
# HONEST DISCLOSURE -- R15 COMPLETES ONE ORDER-LOCK (5 -> 6): the Phenylketonuria
#   (C0031485) S lift to 0.75 [L] makes PKU's three scored axes (O 1.0 [L], D
#   0.50 [L], S 0.75 [L]) all [L] at >=3 axes, so PKU order-locks. This is an
#   honest CONSEQUENCE of a maximally defensible severity lift, NOT lock-fishing:
#   the S and D axes are orthogonal by construction (disease-severity magnitude
#   vs GBD functional-dependence weight), the sources differ (PMC-OA severity
#   literature vs the GBD 2013 disability weight), the cited sentence passes
#   every inclusion criterion independently, and PKU's untreated severity is the
#   single most robustly documented disease-level magnitude in the cohort. PKU
#   locks on the same 3-scored-axes-all-[L] pattern (P,M unscored [O]) as the
#   existing Achondrogenesis type II and Tyrosinemia type II locks. The R15 gate
#   asserts the lock-set delta is EXACTLY {+PKU, -nothing} and that the two other
#   R15 lifts (AIP, Achondroplasia) do NOT lock.
#
# Self-healing / idempotent: if its own pass-tag is already present, R15 re-runs
# r5..r14 to rebuild a clean R14 base, then re-applies. Writes the payload back
# to burden_scores_registry.json (+ burden_residual_registry.{json,csv}) and
# prints the registry-set sha256[:12] over the four emitted artifacts.
# The R15 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# ---- import R14's helpers (which import R13's, ... R9's PURE helpers) so the
#      frozen tier / spectrum / decline logic is literally the same object
#      (cannot drift); r14.main / ... / r9.main are __main__-guarded ----
_s14 = importlib.util.spec_from_file_location("r14_litc", os.path.join(HERE, "r14_severity_litcurate.py"))
r14 = importlib.util.module_from_spec(_s14); _s14.loader.exec_module(r14)
W = r14.W
GRADE_RANK = r14.GRADE_RANK
floor_grade = r14.floor_grade
weighted_burden = r14.weighted_burden
spearman = r14.spearman
RANK_MIN = r14.RANK_MIN
AXIS_PATTERN = r14.AXIS_PATTERN
P_CUTPOINTS = r14.P_CUTPOINTS
S_CUTPOINTS = r14.S_CUTPOINTS
DECLINE_VOCAB = r14.DECLINE_VOCAB
LIT_SRC_TMPL = r14.LIT_SRC_TMPL
derive_tier = r14.derive_tier          # frozen R3 first_match + spectrum guard
load_pinned = r14.load_pinned          # verifies pinned OA cache + its own sha
_append_once = r14._append_once


def load_join7():
    p = os.path.join(METH, "severity_litcurate7_join.csv")
...
def load_excluded7():
    p = os.path.join(METH, "severity_litcurate7_excluded.csv")
    if not os.path.exists(p):
...
def main():
    join = load_join7()
    excluded = load_excluded7()

    base = json.load(open(OUT_SCORES_J))
    # self-healing: if R15 already applied, rebuild a clean R14 base from r5..r14
    if "R15_litcurate7" in base.get("registry_passes", []):
        for stage in ("r5_accession_apply.py", "r6_naturalhistory_registry.py",
                      "r7_naturalhistory_registry2.py", "r8_severity_registry.py",
                      "r9_severity_litcurate.py", "r10_severity_litcurate.py",
                      "r11_severity_litcurate.py", "r12_severity_litcurate.py",
                      "r13_severity_litcurate.py", "r14_severity_litcurate.py"):
            subprocess.run([sys.executable, os.path.join(HERE, stage)],
                           check=True, stdout=subprocess.DEVNULL)
        base = json.load(open(OUT_SCORES_J))
    if "R14_litcurate6" not in base.get("registry_passes", []):
        print("  ERROR: R15 expects an R14 base (registry_passes missing R14_litcurate6)", file=sys.stderr)
        sys.exit(3)
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement (round 7) considered and declined "
    ...
    # ---------------- recompute composite / coverage / rankability (identical to R8..R14) ----------------
    for r in recs:
        comp = r["components"]
    ...
    payload = {
        ...
        "phase": "R15 (curated severity/progression literature, round 7; cumulative over R5+R6+R7+R8+R9+R10+R11+R12+R13+R14)",
        "investigation_only": base.get("investigation_only", True),
        "generated": RETRIEVED,
        "method": (base.get("method", "")
                   + " + curated PMC-OA severity/progression literature join, round 7 (R15)"),
        "derived_from": ("burden_scores_registry.json (R5..R14) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate7_join.csv + methodology/severity_litcurate7_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R15_litcurate7"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R15: SEVENTH curated PMC OPEN-ACCESS S/P round over the SAME "
            "a-priori inclusion criterion and the SAME frozen R3 tier function as R9..R14, applied through a "
            "fresh, separately-auditable curation join (methodology/severity_litcurate7_join.csv) plus a recorded "
            "decline set (methodology/severity_litcurate7_excluded.csv). R15 does NOT modify R9..R14's recorded "
            "join/excluded CSVs or their recorded registry-set shas; it is a separate cumulative stage over the R14 "
            "registry, exactly as R14 was a separate cumulative stage over R13 and R7 over R6. derive_tier / "
            "is_spectrum / the cut-point tables / the decline vocabulary are IMPORTED (through r14_severity_litcurate, "
            "which imports r13_severity_litcurate, ... which imports r9_severity_litcurate) so they cannot drift. "
            "Registry [L] supersedes [H]/[O], existing [L] corroborated (max, add-only). No inference; no new "
            "cut-points. R15 completes ONE order-lock (Phenylketonuria, 5 -> 6) as an honest CONSEQUENCE of a "
            "maximally defensible severity lift on an axis orthogonal to PKU's already-[L] onset and disability axes; "
            "the two other R15 lifts (AIP, Achondroplasia) do not lock. This is disclosed, not engineered, and the "
            "R15 gate asserts the lock-set delta is exactly {+Phenylketonuria}."),
        ...
        "litcurate2_source": base.get("litcurate2_source"),
        "litcurate3_source": base.get("litcurate3_source"),
        "litcurate4_source": base.get("litcurate4_source"),
        "litcurate5_source": base.get("litcurate5_source"),
        "litcurate6_source": base.get("litcurate6_source"),
        "litcurate7_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                              "retrieved": RETRIEVED,
        ...
        "litcurate2_join": base.get("litcurate2_join", []),
        "litcurate2_excluded": base.get("litcurate2_excluded", []),
        "litcurate3_join": base.get("litcurate3_join", []),
        "litcurate3_excluded": base.get("litcurate3_excluded", []),
        "litcurate4_join": base.get("litcurate4_join", []),
        "litcurate4_excluded": base.get("litcurate4_excluded", []),
        "litcurate5_join": base.get("litcurate5_join", []),
        "litcurate5_excluded": base.get("litcurate5_excluded", []),
        "litcurate6_join": base.get("litcurate6_join", []),
        "litcurate6_excluded": base.get("litcurate6_excluded", []),
        "litcurate7_join": join_audit,
        "litcurate7_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R15 (curated severity/progression literature, round 7; cumulative over R5..R14)",
        "derived_from": "burden_scores_registry.json (R5..R15) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R15 curated severity/progression literature pass, ROUND 7 (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R15 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
