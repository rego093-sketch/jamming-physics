#!/usr/bin/env python3
# =============================================================================
# R12 - curated SEVERITY / PROGRESSION literature pass, ROUND 4
#        (cumulative over R5..R11)
# -----------------------------------------------------------------------------
# R9/R10/R11 lifted S/P for the cohort diseases whose dominant untreated sequela
...
# R12 is a FOURTH curation round over the SAME a-priori inclusion criterion and
# the SAME FROZEN R3 tier function -- it does NOT touch R9's, R10's or R11's
# recorded artifacts (methodology/severity_litcurate{,2,3}_join.csv,
# *_excluded.csv) or their recorded registry-set shas; it adds a fresh,
# separately-auditable join/excluded set (methodology/severity_litcurate4_join.csv,
# *_excluded.csv) on top of the R11 registry, exactly as R11 was a separate
# cumulative stage over R10 and R7 was over R6.
#
...
#   THROUGH R11->R10->R9; no new cut-points), the sentence is re-found verbatim in
#   the pinned OA cache at build time or the build fails, and the R3 spectrum
#   override is enforced. All of derive_tier / is_spectrum / the cut-point tables /
#   the decline vocabulary are IMPORTED (through r11_severity_litcurate, which
#   imports r10_severity_litcurate, which imports r9_severity_litcurate) so they
#   cannot drift.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated
#   (max). A missing/declined source NEVER downgrades a value. R12 touches ONLY
#   S and P; O/M/D stay byte-identical to R11.
#
# Self-healing / idempotent: if its own pass-tag is already present, R12 re-runs
# r5..r11 to rebuild a clean R11 base, then re-applies. Writes the payload back to
...
# The R12 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# ---- import R11's helpers (which import R10's, which import R9's PURE helpers)
#      so the frozen tier / spectrum / decline logic is literally the same object
#      (cannot drift); r11.main / r10.main / r9.main are __main__-guarded ----
_s11 = importlib.util.spec_from_file_location("r11_litc", os.path.join(HERE, "r11_severity_litcurate.py"))
r11 = importlib.util.module_from_spec(_s11); _s11.loader.exec_module(r11)
W = r11.W
GRADE_RANK = r11.GRADE_RANK
floor_grade = r11.floor_grade
weighted_burden = r11.weighted_burden
spearman = r11.spearman
RANK_MIN = r11.RANK_MIN
AXIS_PATTERN = r11.AXIS_PATTERN
P_CUTPOINTS = r11.P_CUTPOINTS
S_CUTPOINTS = r11.S_CUTPOINTS
DECLINE_VOCAB = r11.DECLINE_VOCAB
LIT_SRC_TMPL = r11.LIT_SRC_TMPL
derive_tier = r11.derive_tier          # frozen R3 first_match + spectrum guard
load_pinned = r11.load_pinned          # verifies pinned OA cache + its own sha
_append_once = r11._append_once


def load_join4():
    p = os.path.join(METH, "severity_litcurate4_join.csv")
...
def load_excluded4():
    p = os.path.join(METH, "severity_litcurate4_excluded.csv")
    if not os.path.exists(p):
...
def main():
    join = load_join4()
    excluded = load_excluded4()

    base = json.load(open(OUT_SCORES_J))
    # self-healing: if R12 already applied, rebuild a clean R11 base from r5..r11
    if "R12_litcurate4" in base.get("registry_passes", []):
        for stage in ("r5_accession_apply.py", "r6_naturalhistory_registry.py",
                      ...
                      "r11_severity_litcurate.py"):
            subprocess.run([sys.executable, os.path.join(HERE, stage)],
                           check=True, stdout=subprocess.DEVNULL)
        base = json.load(open(OUT_SCORES_J))
    if "R11_litcurate3" not in base.get("registry_passes", []):
        print("  ERROR: R12 expects an R11 base (registry_passes missing R11_litcurate3)", file=sys.stderr)
        sys.exit(3)
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement (round 4) considered and declined "
    ...
    # ---------------- recompute composite / coverage / rankability (identical to R8/R9/R10/R11) ----------------
    for r in recs:
        comp = r["components"]
    ...
    payload = {
        ...
        "phase": "R12 (curated severity/progression literature, round 4; cumulative over R5+R6+R7+R8+R9+R10+R11)",
        "investigation_only": base.get("investigation_only", True),
        "generated": RETRIEVED,
        "method": (base.get("method", "")
                   + " + curated PMC-OA severity/progression literature join, round 4 (R12)"),
        "derived_from": ("burden_scores_registry.json (R5..R11) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate4_join.csv + methodology/severity_litcurate4_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R12_litcurate4"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R12: FOURTH curated PMC OPEN-ACCESS S/P round over the SAME "
            "a-priori inclusion criterion and the SAME frozen R3 tier function as R9/R10/R11, applied through a "
            "fresh, separately-auditable curation join (methodology/severity_litcurate4_join.csv) plus a recorded "
            "decline set (methodology/severity_litcurate4_excluded.csv). R12 does NOT modify R9's, R10's or R11's "
            "recorded join/excluded CSVs or their recorded registry-set shas; it is a separate cumulative stage over "
            "the R11 registry, exactly as R11 was a separate cumulative stage over R10 and R7 over R6. derive_tier / "
            "is_spectrum / the cut-point tables / the decline vocabulary are IMPORTED (through r11_severity_litcurate, "
            "which imports r10_severity_litcurate, which imports r9_severity_litcurate) so they cannot drift. Registry "
            "[L] supersedes [H]/[O], existing [L] corroborated (max, add-only). No inference; no new cut-points."),
        ...
        "litcurate4_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                              "retrieved": RETRIEVED,
        ...
        "litcurate4_join": join_audit,
        "litcurate4_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R12 (curated severity/progression literature, round 4; cumulative over R5..R11)",
        "derived_from": "burden_scores_registry.json (R5..R12) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R12 curated severity/progression literature pass, ROUND 4 (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R12 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
