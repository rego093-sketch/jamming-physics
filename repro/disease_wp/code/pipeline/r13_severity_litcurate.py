#!/usr/bin/env python3
# =============================================================================
# R13 - curated SEVERITY / PROGRESSION literature pass, ROUND 5
#        (cumulative over R5..R12)
# -----------------------------------------------------------------------------
# R9/R10/R11/R12 lifted S/P for the cohort diseases whose dominant untreated
...
# order-lock. R13 is a FIFTH curation round over the SAME a-priori inclusion
# criterion and the SAME FROZEN R3 tier function -- it does NOT touch R9's,
# R10's, R11's or R12's recorded artifacts (methodology/
# severity_litcurate{,2,3,4}_join.csv, *_excluded.csv) or their recorded
# registry-set shas; it adds a fresh, separately-auditable join/excluded set
# (methodology/severity_litcurate5_join.csv, *_excluded.csv) on top of the R12
# registry, exactly as R12 was a separate cumulative stage over R11 and R7 was
# over R6.
#
...
#   THROUGH R12->R11->R10->R9; no new cut-points), the sentence is re-found
...
#   r12_severity_litcurate, which imports r11_severity_litcurate, which imports
#   r10_severity_litcurate, which imports r9_severity_litcurate) so they cannot
#   drift.
#
# add-only: registry [L] supersedes [H]/[O]; an existing [L] is corroborated
#   (max). A missing/declined source NEVER downgrades a value. R13 touches ONLY
#   S and P; O/M/D stay byte-identical to R12.
#
# Self-healing / idempotent: if its own pass-tag is already present, R13 re-runs
# r5..r12 to rebuild a clean R12 base, then re-applies. Writes the payload back
...
# The R13 pipeline is LIVING CODE and is intentionally NOT in MANIFEST_governed.
# =============================================================================
...
# ---- import R12's helpers (which import R11's, R10's, R9's PURE helpers)
#      so the frozen tier / spectrum / decline logic is literally the same object
#      (cannot drift); r12.main / r11.main / r10.main / r9.main are __main__-guarded ----
_s12 = importlib.util.spec_from_file_location("r12_litc", os.path.join(HERE, "r12_severity_litcurate.py"))
r12 = importlib.util.module_from_spec(_s12); _s12.loader.exec_module(r12)
W = r12.W
GRADE_RANK = r12.GRADE_RANK
floor_grade = r12.floor_grade
weighted_burden = r12.weighted_burden
spearman = r12.spearman
RANK_MIN = r12.RANK_MIN
AXIS_PATTERN = r12.AXIS_PATTERN
P_CUTPOINTS = r12.P_CUTPOINTS
S_CUTPOINTS = r12.S_CUTPOINTS
DECLINE_VOCAB = r12.DECLINE_VOCAB
LIT_SRC_TMPL = r12.LIT_SRC_TMPL
derive_tier = r12.derive_tier          # frozen R3 first_match + spectrum guard
load_pinned = r12.load_pinned          # verifies pinned OA cache + its own sha
_append_once = r12._append_once


def load_join5():
    p = os.path.join(METH, "severity_litcurate5_join.csv")
...
def load_excluded5():
    p = os.path.join(METH, "severity_litcurate5_excluded.csv")
    if not os.path.exists(p):
...
def main():
    join = load_join5()
    excluded = load_excluded5()

    base = json.load(open(OUT_SCORES_J))
    # self-healing: if R13 already applied, rebuild a clean R12 base from r5..r12
    if "R13_litcurate5" in base.get("registry_passes", []):
        ...
        base = json.load(open(OUT_SCORES_J))
    if "R12_litcurate4" not in base.get("registry_passes", []):
        print("  ERROR: R13 expects an R12 base (registry_passes missing R12_litcurate4)", file=sys.stderr)
        sys.exit(3)
    ...
    for cui, rows in excluded.items():
        ...
        for x in rows:
            ...
            ax = rec_by_cui[cui]["components"][axis]
            note = (f"curated-literature {axis}-magnitude statement (round 5) considered and declined "
    ...
    payload = {
        ...
        "phase": "R13 (curated severity/progression literature, round 5; cumulative over R5+R6+R7+R8+R9+R10+R11+R12)",
        "investigation_only": base.get("investigation_only", True),
        "generated": RETRIEVED,
        "method": (base.get("method", "")
                   + " + curated PMC-OA severity/progression literature join, round 5 (R13)"),
        "derived_from": ("burden_scores_registry.json (R5..R12) + data/raw/litcurate/ (PMC-OA, pinned) + "
                         "methodology/severity_litcurate5_join.csv + methodology/severity_litcurate5_excluded.csv"),
        "registry_passes": list(base.get("registry_passes", [])) + ["R13_litcurate5"],
        "weights_default": W,
        ...
        "lift_policy": (
            base.get("lift_policy", "") + "  ||  R13: FIFTH curated PMC OPEN-ACCESS S/P round over the SAME "
            "a-priori inclusion criterion and the SAME frozen R3 tier function as R9/R10/R11/R12, applied through a "
            "fresh, separately-auditable curation join (methodology/severity_litcurate5_join.csv) plus a recorded "
            "decline set (methodology/severity_litcurate5_excluded.csv). R13 does NOT modify R9's, R10's, R11's or "
            "R12's recorded join/excluded CSVs or their recorded registry-set shas; it is a separate cumulative stage "
            "over the R12 registry, exactly as R12 was a separate cumulative stage over R11 and R7 over R6. "
            "derive_tier / is_spectrum / the cut-point tables / the decline vocabulary are IMPORTED (through "
            "r12_severity_litcurate, which imports r11_severity_litcurate, which imports r10_severity_litcurate, which "
            "imports r9_severity_litcurate) so they cannot drift. Registry [L] supersedes [H]/[O], existing [L] "
        ...
        "litcurate5_source": {"dir": "data/raw/litcurate/", "class": "PubMed Central open-access full text",
                              "retrieved": RETRIEVED,
        ...
        "litcurate5_join": join_audit,
        "litcurate5_excluded": excluded_audit,
    ...
    rj = {
        ...
        "phase": "R13 (curated severity/progression literature, round 5; cumulative over R5..R12)",
        "derived_from": "burden_scores_registry.json (R5..R13) + treatments_registry.csv (R4/R5 offset)",
    ...
    # ----- report -----
    print("--- R13 curated severity/progression literature pass, ROUND 5 (PMC-OA dominant-sequela join) ---")
    print(f"  curation lifts:  {len(join_audit)}   declines recorded: {len(excluded_audit)}")
    print("\n  lift events:")
    for k in sorted(lift):
        print(f"    {k:34s} {lift[k]}")
    print("\n  S/P axis grade coverage AFTER R13 (over 35):")
    print(f"    S: {cov['S']}")
    print(f"    P: {cov['P']}")
    if per_axis_changes:
