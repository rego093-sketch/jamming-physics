...
def floor_retest_ablation():
    ...
    is over the anchored genes present in that scope. [L] (empirical, scope-localised)."""
    anc = lock.carnegie_anchor()
    ...
    return {
        ...
        "grade": "[L] the ABSOLUTE depth<->Carnegie strength now REACHES the pre-registered 0.70 "
                 "floor globally on the filled kit (~0.755 vs 0.475 inherited). The lift localises "
                 "to the named dilutors: the limb-inducer fix is cleanly independent and alone "
                 "reaches the floor; HOX adds collinear lateness. Empirical [L], not [V].",
    }


def floor_retest_jitter():
    """JITTER ROBUSTNESS of the filled-kit floor: perturb every Carnegie rank by +-1 (seed-locked)
    and report the distribution of Spearman(depth, jittered rank). Honest about the lower edge."""
    anc = lock.carnegie_anchor()
    d = cascade.depth()
    ...
    return {
        ...
        "floor": floor,
        "honest_lower_edge": ("the floor is cleared by the cited ranks (mean %.3f) but is MARGINAL "
                              "at the lower edge of citation uncertainty (p5 %.3f < %.2f), so only "
                              "%.0f%% of jittered draws stay at or above it."
                              % (mean, p5, floor, 100.0 * frac)),
        "grade": "[L] the filled-kit floor is cleared on average under +-1 rank jitter but marginal "
                 "at the p5 lower edge -- reported honestly; the absolute-strength claim stays [L], "
                 "not [V].",
    }
