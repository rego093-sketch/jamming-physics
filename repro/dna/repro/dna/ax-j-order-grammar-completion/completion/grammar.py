...
def order_to_days():
    """O2: the order grammar, converted to approximate DAYS by the real measured segmentation-clock
    period + Carnegie stage-day table (completion.timing). Sub-clock [L]; single zero-point [O]."""
    ...
    return {
        ...
        "global_zero_point": glob["what_stays_open"],
        "grade": "[L] the segmentation sub-clock predicts a somitogenesis span consistent with the "
                 "cited Carnegie window; [O] one global zero-point that pins ALL genes to absolute "
                 "days is still open (would need a second measured anchor).",
    }
