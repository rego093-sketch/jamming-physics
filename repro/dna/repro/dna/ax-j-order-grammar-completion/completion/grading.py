# -*- coding: utf-8 -*-
"""
completion.grading -- the honest claim-by-claim ledger for Appendix J.

Every claim the completion makes, with its grade and the function that backs it. The gate audits
that nothing graded [V] is unsupported and -- critically -- that the O1 absolute-strength row stays
[L] and is never quietly upgraded to [V] just because the floor is now met.
...
"""
from . import lock, cascade, coupled, nulltest, timing, declaration
...
def ledger():
    ...
    rows = [
        ...
        {"id": "O1", "claim": "ABSOLUTE depth<->Carnegie strength REACHES the pre-registered 0.70 "
         ...
         "grade": "[L] met=%s (%.3f); empirical/jitter-sensitive -- not a logical invariant"
         ...
         "backing": "nulltest.floor_retest_ablation (inherited %.3f -> filled %.3f; limb-only %.3f, "
                    "HOX-only %.3f) + floor_retest_jitter (mean %.3f, p5 %.3f, frac>=floor %.3f)"
        ...
        {"id": "O2b", "claim": "ONE global zero-point pinning ALL genes to absolute days",
         "grade": "[O] open", "backing": "timing.global_clock_open: needs a 2nd measured anchor"},
        # ---- scope ----
