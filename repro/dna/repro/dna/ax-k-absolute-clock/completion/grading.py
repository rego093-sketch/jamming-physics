# -*- coding: utf-8 -*-
"""
completion.grading -- the honest claim-by-claim ledger for Appendix K (the absolute clock).

Every claim the appendix makes, with its grade and the function that backs it. The gate audits that
nothing graded [V] is unsupported and -- critically -- that BOTH the O1 absolute-strength row AND the
O2b absolute-clock row stay [L], and are never quietly upgraded to [V] just because they now succeed.
