#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gates.py  --  Special-Sense Organs research/writing PHASE gate (VP-SPEC C1/C3 + research-first)."""
import os, sys, json
...
def writing_locked():
    # 'published' is a strict superset of 'writing': everything true in writing still holds, plus the
    # concept DOI is assigned and the Zenodo-deposit artifacts (PDF/TeX) are shipped. Research must still
    # be signed off (research_complete.json, all_green) -- broadening the unlock set weakens no safety gate.
    if read_phase() not in ("writing", "published"): return True, "PHASE not in {writing, published}"
    ...
    return False, "writing/publication unlocked"
