...
Reads  seed.json  (the package's self-description) and asserts these, in order:
  ...
  [5] VOLUME REPRODUCES.  The published HTML volume (docs/nose/) is regenerated-and-checked by its own
      focused gate (tools/gate_volume.py): the numeric SSOT (tools/vp_nose_ssot.py) is deterministic
      (run twice → identical sha256), every displayed number is sourced from that SSOT so HTML↔code drift
      is exactly 0, each chapter carries its required structure, both disease chapters carry the
      non-clinical direction-only scope banner, and the access layer (sitemap/robots/llms) is consistent.
      This step touches only the seed's own tools — no inherited/ file is read or modified.
...
def main():
    ...
    # [5] VOLUME REPRODUCES ------------------------------------------------------------
    print("\n[5] volume reproduces — published HTML volume regenerated + drift-checked (docs/nose/):")
    gate = os.path.join(ROOT, "tools", "gate_volume.py")
    if not os.path.exists(gate):
        ok_all = False
        print("    [FAIL] tools/gate_volume.py missing")
    else:
        r = subprocess.run([sys.executable, gate], capture_output=True, text=True, cwd=ROOT)
        passed = (r.returncode == 0) and ("VOLUME GATE: PASS" in r.stdout)
        ok_all &= passed
        # surface the per-check PASS/FAIL lines from the focused gate, indented under this step
        for l in r.stdout.splitlines():
            s = l.strip()
            if s.startswith("[PASS]") or s.startswith("[FAIL]"):
                print(f"    {s}")
        print(f"    [{'PASS' if passed else 'FAIL'}] tools/gate_volume.py — "
              f"{'VOLUME GATE: PASS (SSOT deterministic, HTML↔code drift 0)' if passed else 'volume gate did not pass'}")
        if not passed and r.stderr.strip():
