...
# -*- coding: utf-8 -*-
"""
fetch_rna_gamma.py  --  small-RNA / transgenerational-RNA machinery promoter gamma RECEPTION.
...
MODES
  (default, OFFLINE)  verify() : recompute gamma from inherited/rna_carrier_promoters.cache.json,
                                 cross-check the SOX9 anchor reproduces, confirm agreement with
                                 inherited/rna_carrier_gamma.json. Deterministic, no network.
...
_CACHE = os.path.join(_INH, "rna_carrier_promoters.cache.json")
_ATLAS = os.path.join(_INH, "rna_carrier_gamma.json")
...
PANEL = {
    "DROSHA":  "microprocessor RNase III (nuclear pri-miRNA cleavage)",
    "DGCR8":   "microprocessor partner (pri-miRNA recognition)",
    "DICER1":  "cytoplasmic RNase III (pre-miRNA -> mature miRNA/siRNA)",
    "AGO2":    "Argonaute-2 (RISC catalytic slicer)",
    "TARBP2":  "TRBP (RISC loading / Dicer cofactor)",
    "PIWIL1":  "PIWI (piRNA pathway, germline silencing)",
    "MOV10L1": "germline RNA helicase (piRNA biogenesis)",
    "DDX4":    "VASA germ-granule RNA helicase (germline RNA)",
    "METTL3":  "m6A methyltransferase (RNA epigenetic writer)",
    "YTHDF2":  "m6A reader (RNA-mark decay)",
    "ELAVL1":  "HuR mRNA-stability RNA-binding protein",
    "ANG":     "angiogenin (tRNA cleavage -> sperm tsRNA/tRF carriers)",
}
...
def fetch():
    ...
    cache = {
        "_what": "Cached human proximal-promoter sequences (GRCh38, NCBI exact TSS, window TSS-2000..+500) "
                 "for the small-RNA / transgenerational-RNA machinery. Offline bit-for-bit gamma reproduction.",
        ...
        "_panel_declaration": "Panel declared by RNA-machinery function BEFORE gamma was seen.",
        "_anchor_ok": True,
        "genes": cache_genes,
    }
    json.dump(cache, open(_CACHE, "w", encoding="utf-8"), ensure_ascii=False, indent=1, sort_keys=True)
    atlas = {"_atlas": "VP small-RNA / transgenerational-RNA machinery master-gene gamma (NCBI-direct).",
             ...
             "genes": {k: {"gamma": v["gamma"], "gc": v["gc"], "role": v["role"],
...
def main():
    ap = argparse.ArgumentParser(description="RNA-machinery promoter gamma reception (NCBI-direct).")
