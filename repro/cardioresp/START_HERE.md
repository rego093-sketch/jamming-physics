# START HERE — Cardiorespiratory (cardioresp_vp_site)  ·  v0.1.0-research
...
|---|---|---|---|
| NKX2-5 | heart | SA-node pacemaker + conduction + contraction | oscillator |
| NKX2-1 | lung | preBotzinger inspiratory rhythm + ventilatory pump | oscillator |
...
- **lung carcinoma** ← tobacco smoke condensate; radon progeny; PM2.5/diesel  · anchor: RR vs pack-years (Doll & Peto class) [L]; near-linear no-threshold shape [V]
- **(heart: sarcoma rare)** ← n/a primary; baseline incidence only  · anchor: background crossing rate only; not a dose-response target
...
├── FUTURE_WORK.md           ← 추후 연구 로드맵 (호흡기 질환 확장: 천식·OSA·감기/발열·기침 등; 미검증 가설, docs/ 에 미포함)
...
│   └── cardiac_rhythm_anchor.json  ← 심박 앵커 (mind interoception_atlas에서)
├── repro/
│   ├── _engine/vp_car_engine.py   ← 장기 창발 + 순환 (창발은 지금 실행; 동역학은 골격)
...
├── manifest/cardioresp_vp_site.csv
