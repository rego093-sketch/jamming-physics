...
# -*- coding: utf-8 -*-
"""
depression_forbidden_claim_scan.py  —  the discipline guard (fail-closed). Inherited from analgesic
v2.0 M5 via the bipolar/epilepsy T-L scanners.

Scans the ASSERTED claims of the depression three-lever map + the burden prioritisation for FORBIDDEN
...
into a treatment / drug / efficacy claim -- which matters ACUTELY here because the antidepressant
literature (SSRIs, SNRIs, MAOIs, ketamine/esketamine, the neurosteroids, ezogabine) is saturated with
efficacy / "remission" / "relapse-prevention" / "lifts depression" language that must never leak into
the asserted map.

What is SCANNED (positive assertions only):
  - depression_threshold_levers_results.json   (agent direction, anchors, frame text, entries)
  - depression_burden_prioritisation.json       (the ranking rows + reading)
...
Run:  python3 depression_forbidden_claim_scan.py   -> depression_claim_scan.json ; exit 1 on violation
"""
import os, re, sys, json
HERE = os.path.dirname(os.path.abspath(__file__))

SCAN_FILES = ["depression_threshold_levers_results.json", "depression_burden_prioritisation.json"]
...
PATTERNS = {
    ...
    "EFFICACY_AS_FACT": [r"\bcures\b", r"\bwill\s+cure\b", r"\btreats?\s+patients?\b",
                         ...
                         # depression-specific efficacy-as-fact:
                         r"\blifts?\s+(depression|mood|the\s+mood)\b", r"\bprevents?\s+relapse\b",
                         r"\b(achieves?|induces?|produces?)\s+remission\b", r"\bremission\b",
                         r"\bcures?\s+depression\b", r"\btreats?\s+depression\b",
                         r"\b(relieves?|resolves?|eliminates?)\s+depression\b",
                         r"\bdepression[-\s]?free\b", r"\bantidepressant\s+efficacy\b",
                         r"\bimproves?\s+depression\b", r"\brestores?\s+mood\b"],
...
if __name__ == "__main__":
    ...
    planted = "Give sertraline 50 mg twice daily; this cures depression, lifts mood, prevents relapse and is completely safe."
    ...
    result = {"inherited_from": "analgesic_threshold_logic v2.0 M5 (DOI 10.5281/zenodo.20733420) via bipolar/epilepsy T-L",
    ...
    json.dump(result, open(os.path.join(HERE, "depression_claim_scan.json"), "w"), indent=1)

    print("forbidden-claim scan (depression)")
