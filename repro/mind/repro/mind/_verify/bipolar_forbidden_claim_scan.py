...
# -*- coding: utf-8 -*-
"""
bipolar_forbidden_claim_scan.py  —  the discipline guard (fail-closed). Inherited from analgesic v2.0 M5.

Scans the ASSERTED claims of the bipolar three-lever map + the burden prioritisation for FORBIDDEN
...
into a treatment / drug / efficacy claim -- which matters acutely here because the mood-stabiliser
and lithium literature is full of efficacy language that must never leak into the asserted map.

What is SCANNED (positive assertions only):
  - bipolar_threshold_levers_results.json   (push_direction, anchors, frame text, entries)
  - bipolar_burden_prioritisation.json      (the ranking rows + reading)
...
Run:  python3 bipolar_forbidden_claim_scan.py   -> bipolar_claim_scan.json ; exit 1 on any violation
"""
import os, re, sys, json
HERE = os.path.dirname(os.path.abspath(__file__))

SCAN_FILES = ["bipolar_threshold_levers_results.json", "bipolar_burden_prioritisation.json"]

PATTERNS = {
    "DOSING": [r"\b\d+(\.\d+)?\s?mg\b", r"\bmg\s*/\s*kg\b", r"\bdosage\b", r"\bdose[sd]?\b",
               r"\btwice\s+daily\b", r"\bonce\s+daily\b", r"\b[bt]id\b", r"\bqd\b",
               r"\bmilligram", r"\bper\s+kg\b", r"\bserum\s+level\b"],
    ...
    "EFFICACY_AS_FACT": [r"\bcures\b", r"\bwill\s+cure\b", r"\btreats?\s+patients?\b",
                         r"\bproven\s+to\s+(relieve|prevent|stabilise|stabilize)\b", r"\bguarantee",
                         r"\bis\s+effective\s+in\s+patients\b", r"\bprevents?\s+(mania|episodes?|relapse)\b",
                         r"\bstops?\s+(mania|episodes?)\b"],
...
if __name__ == "__main__":
    ...
    planted = "Give lithium 900 mg twice daily; this cures bipolar, prevents mania and is completely safe."
    ...
    result = {"inherited_from": "analgesic_threshold_logic v2.0 M5 (DOI 10.5281/zenodo.20733420)",
    ...
    json.dump(result, open(os.path.join(HERE, "bipolar_claim_scan.json"), "w"), indent=1)

    print("forbidden-claim scan (bipolar)")
