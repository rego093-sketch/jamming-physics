...
# -*- coding: utf-8 -*-
"""
falsification.py  --  IV falsification register (mirror of analgesic M6).
A proposal you cannot kill is not science. Gate: every HP proposal id has >=1 named, measurable
falsifier, plus a framework-level falsifier. Ported from analgesic_threshold_logic v2.0 (Zenodo
concept DOI 10.5281/zenodo.20733420).
Run:  python3 falsification.py  -> expected/comfort_falsification.json
"""
import os, json
HERE = os.path.dirname(os.path.abspath(__file__))
PROP = os.path.join(HERE, "comfort_proposal.json")
...
def register():
    ...
    return {"title": "Comfort-logic falsification register",
            "imported_from": "analgesic_threshold_logic v2.0 M6 (Zenodo concept DOI 10.5281/zenodo.20733420)",
            "falsifiers": FALSIFIERS, "proposal_ids": ids, "missing": missing,
            "overall": "PASS" if not missing else "FAIL"}


if __name__ == "__main__":
    out = register()
    ids = out["proposal_ids"]; missing = out["missing"]
    json.dump(out, open(os.path.join(HERE, "expected", "comfort_falsification.json"), "w", encoding="utf-8"),
              ensure_ascii=False, indent=1)
    print("IV falsification register")
    for i in ids:
        print("  [%s] %s: %s..." % ("OK" if i in FALSIFIERS else "MISSING", i, FALSIFIERS.get(i, "-")[:78]))
