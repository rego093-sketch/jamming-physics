...
The R19 jamming-lattice bistable switch + the FitzHugh-Nagumo relaxation oscillator (Neuron) + the
organ-emergence helpers (Organ / spinodal / dwell). Single source of the substrate math across the
VP papers; vendored so this package reproduces offline. Do not rewrite the math (VP-SPEC C1).

For this package the SAME primitive carries two extra readings: a SETPOINT-defending loop is an R19
attractor whose basin resists environmental drive (endothermy), and a switch between two regulated
attractors past a spinodal is a discrete state flip (the torpor/hibernation question).
