#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""run_all.py  --  Aging / Senescence RESEARCH entry. Drop into a fresh chat: python repro/run_all.py"""
import os, sys, json
_HERE = os.path.dirname(os.path.abspath(__file__))
for sub in ("_engine", "_verify", "_pathology"): sys.path.insert(0, os.path.join(_HERE, sub))
import importlib
eng = importlib.import_module("vp_age_engine"); stress = importlib.import_module("stress_tests")
gates = importlib.import_module("gates"); path = importlib.import_module("setpoint_failure")

def main():
    print("=" * 78); print("Aging / Senescence  --  RESEARCH PHASE (writing is locked until gates are green)"); print("=" * 78)
